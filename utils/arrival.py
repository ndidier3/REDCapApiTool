...
def get_time_difference_hours(df, variables):
  ...
  new_column = (pd.to_datetime(df[arrival_time]) - pd.to_datetime(df[timestamp])) / np.timedelta64(1, 'h')
  df.insert(column_index+1, variable_name, new_column.round(2))
  return df

def get_time_difference_days(df, variables):
  variable_name, arrival_time, timestamp = variables[0], variables[1], variables[2]
  column_index = df.columns.get_loc(timestamp)
  new_column = (pd.to_datetime(df[arrival_time]) - pd.to_datetime(df[timestamp])) / np.timedelta64(1, 'D')
  # THIS NEEDS TO BE REDUCED BY 24 HOURS (OR 12)
  df.insert(column_index+1, variable_name, new_column.round(2))
  return df
...
def get_sleep_hours(df, variables):
  ...
  to_assess = df[~falsy]
  to_ignore = df[falsy]
  new_column = (pd.to_datetime(to_assess[bed_time], format="%H:%M") - pd.to_datetime(to_assess[wake_time], format="%H:%M")).dt.total_seconds() / 60 / 60
  to_assess.insert(column_index+1, variable_name, new_column.round(2))
...
def get_awake_hours(df, variables):
    ...
    new_column = (pd.to_datetime(to_assess[arrival_time]) - wake_datetime).dt.total_seconds() / 60 / 60
    to_assess.insert(column_index+1, variable_name, new_column.round(2))
...
def get_proportion(df, variables):
    ...
    #for menstrual, only consider those not post-menapause
    return df
...
variable_names = {
  ...
  'psych': {
    'redcap_variables': {
        'psyc_endorse': 'AnyPsych_Current',
        'psy_conditions___0': 'Psychotic_Current',
        'psy_conditions___1': 'ODC_Current',
        'psy_conditions___2': 'Mood_Current',
        'psy_conditions___3': 'Eating_Current',
        'psyhx_anx': 'Anx_Hx',
        'psyhx_aud': 'AUD_Hx',
        'psyhx_ed': 'ED_Hx',
        'psyhx_mdd': 'MDD_Hx',
        'psyhx_ocd': 'OCD_Hx',
        'psyhx_ptsd': 'PTSD_Hx',
        'psyhx_schiz': 'Schiz_Hx',
        'psyhx_sud': 'SUD_Hx',
        'psyhealthnotes':'Psych_Notes',
        'ocdscreen1': 'OCD_Screen',
        'bipolarscreen1': 'Bipol_Screen',
        'eatingscreen1': 'Anorex_Screen',
        'eatingscreen2': 'BingeED_Screen',
        'mdescreen1': 'Depr_Screen',
        'mdescreen2': 'Anhed_Screen',
        'ocdscreen1': 'Obsess_Screen',
        'ocdscreen2': 'Compuls_Screen',
        'psychoticscreen1': 'Psychosis_Screen',
    },
    'new_variables': {
    }
  }
